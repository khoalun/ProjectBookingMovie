...
export default function ExLodashTailwind() {
  const arrId = _.uniqBy(data, "maDanhMuc");
  const [id, setId] = useState(arrId[0].maDanhMuc);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-6">
        <div className="h-full p-3 space-y-2 col-span-1 dark:bg-coolGray-900 dark:text-coolGray-100">
            ...
            <ul className="pt-2 pb-4 space-y-1 text-sm">
              {arrId.map((item, index) => {
                const classNameActive = [
                  ...
                  "space-x-3",
                  "rounded-md",
                  id === item.maDanhMuc ? "bg-black text-white" : "",
                ];
                return (
                  <li
        ...
        <div className="col-span-5">
          <h3 className="text-purple-500 text-center mt-5 text-8xl">Menu</h3>
          <section className="text-gray-600 body-font">
            <div className="container px-5 py-24 mx-auto">
              <div className="flex flex-wrap -m-4">
                {_.filter(data, (item) => item.maDanhMuc === id).map(
                  (item, index) => {
                    return (
                      <div className="p-4 lg:w-1/3" key={index}>
                        <div className="h-full bg-gray-100 bg-opacity-75 px-8 pt-16 pb-24 rounded-lg overflow-hidden text-center relative">
                          ...
                          <div className="text-right  leading-none flex justify-end absolute bottom-0 left-0 w-full ">
                            <span className=" text-gray-400 mr-3 inline-flex items-center leading-none text-sm  border-r-2 border-gray-200 text-2xl">
                              Gia : {item.gia}
                            </span>
                          </div>
                        </div>
